...
//
    ...
    var _HashHTMLBlocks = function(text) {
        ...
        */
        text = text
            .replace(/^(<(p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math|style|section|header|footer|nav|article|aside)\b[^\r]*?.*<\/\2>[ \t]*(?=\n+)\n)/gm, hashElement);
        ...
        */
        list_str = list_str
            .replace(/(\n)?(^[ \t]*)([*+-]|\d+[.])[ \t]+([^\r]+?(\n{1,2}))(?=\n*(~0|\2([*+-]|\d+[.])[ \t]+))/gm,
                function(wholeMatch, m1, m2, m3, m4) {
                    var item = m4;
                    const leading_line = m1;
                    const leading_space = m2;
...
//
        ...
        for (var i = 0; i < end; i++) {
            ...
            while (grafsOut[i].search(/~K(\d+)K/) >= 0) {
                let blockText = g_html_blocks[RegExp.$1];
...
// In javascript we're less fortunate.
        ...
        text = text.replace(/~B(.+?)~A/g,
            function(wholeMatch, m1, m2) {
                var leadingText = m1;
                const numSpaces = 4 - leadingText.length % 4; // attacklab: g_tab_width
